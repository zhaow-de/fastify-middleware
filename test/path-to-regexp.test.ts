...
import * as pathToRegexp from '../src/path-to-regexp';

type Test = [
  string,
  (pathToRegexp.TokensToRegexpOptions & pathToRegexp.ParseOptions) | undefined,
  pathToRegexp.Token[],
  Array<[string, (string | undefined)[] | null, pathToRegexp.Match?, pathToRegexp.RegexpToFunctionOptions?]>,
...
const TESTS: Test[] = [
  ...
  [
    ...
    [
      ...
      ['/caf%C3%A9', ['/caf%C3%A9', 'caf%C3%A9'], { path: '/caf%C3%A9', index: 0, params: { test: 'caf%C3%A9' } }],
      [
        '/caf%C3%A9',
        ['/caf%C3%A9', 'caf%C3%A9'],
  ...
  [
    ...
    [
      [{ test: 'route' }, '/route'],
      [{ test: 'something/else' }, '/something%2Felse', { encode: encodeURIComponent }],
      [{ test: 'something/else/more' }, '/something%2Felse%2Fmore', { encode: encodeURIComponent }],
    ],
  ],
  [
    '/:test',
  ...
  [
    ...
    [
      ...
      [{ test: 'abc/123/456' }, '/abc%2F123%2F456', { encode: encodeURIComponent }],
    ],
  ],
  [
    '/:route([a-z]+)',
  ...
  ['/café', { encode: encodeURI }, ['/café'], [['/caf%C3%A9', ['/caf%C3%A9']]], [[null, '/café']]],
  [
    'packages/',
    undefined,
    ['packages/'],
      ...
      describe(util.inspect(path), function () {
        ...
        describe('match' + (opts ? ' using ' + util.inspect(opts) : ''), function () {
          matchCases.forEach(function (io) {
            ...
            const message = `should ${matches ? '' : 'not '}match ${util.inspect(pathname)}`;
