 ...
 */
export function match<P extends object = object>(str: string, options?: ParseOptions & TokensToRegexpOptions & RegexpToFunctionOptions) {
  const keys: Key[] = [];
 ...
 * `keys` will contain `[{ name: 'id', delimiter: '/', optional: false, repeat: false }]`.
 */
export function pathToRegexp(path: string, keys?: Key[], options?: TokensToRegexpOptions & ParseOptions) {
